...
if (! this.binding.variables.containsKey("transaction") || ! transaction) {
    ...
    for (iface in node.getSnmpInterfaces()) {
        if (node?.getSysObjectId() != null && node?.getSysObjectId()?.matches("^\\.1\\.3\\.6\\.1\\.4\\.1\\.2281\\.1\\.7\\..*")) {
            if (iface?.getIfAlias()?.length() == 0 && iface?.getIfName()?.length() == 0 && iface?.getIfDescr()?.length() > 0) { //If this is a Ceragon IP10 with blank ifName and ifAlias on an SNMP interface
